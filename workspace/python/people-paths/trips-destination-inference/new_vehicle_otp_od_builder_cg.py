...
def get_gtfs_path(query_date):
    INTERMEDIATE_OTP_DATE = pd.to_datetime("2019-03-15", format="%Y-%m-%d")
    router_id = ''
...
def choose_leg_matches(leg_matches_groups):
    ...
    for name, group in leg_matches_groups:
        ...
        if (curr_leg_mode == 'WALK'):
            ...
            filtered_group = group[pd.to_datetime(group['bt_start_time']) > prev_leg_end_time]
...
for file in os.listdir(otp_suggestions_folderpath):
    ...
    try:
        ...
        if len(otp_suggestions_raw) != 0:
            ...
            print ("Processing bus File:", bus_trips_filepath)
            ...
            bus_trips_clean = bus_trips.filter(
                ['route', 'busCode', 'tripNum', 'stopPointId', 'gps_datetime'])

            bus_trips_clean.drop(
                bus_trips_clean.loc[bus_trips_clean['gps_datetime'] == '-'].index, inplace=True)
        ...
        # Identify Possible Matches between OTP Itineraries and Bus Trips Observed Data
            ...
            scheduled_itin_observed_o = otp_suggestions_bus_legs.merge(bus_trips_clean.add_prefix('bt_'),
                ...
                .assign(sched_obs_start_timediff=lambda x: np.absolute(pd.to_datetime(x['bt_start_time']).subtract(x['otp_start_time'])))
            ...
            scheduled_itin_observed_od = scheduled_itin_observed_o.merge(bus_trips_clean.add_prefix('bt_'),
                                                                         left_on=[
                                                                             'otp_route', 'bt_bus_code', 'bt_trip_num', 'otp_to_stop_id'],
                                                                         right_on=[
                                                                             'bt_route', 'bt_busCode', 'bt_tripNum', 'bt_stopPointId'],
                                                                         how='inner') \
                                                                    .drop(['bt_route', 'bt_stopPointId'], axis=1) \
                                                                    .rename(index=str, columns={'bt_gps_datetime': 'bt_end_time'}) \
                                                                    .assign(sched_obs_end_timediff=lambda x: np.absolute(pd.to_datetime(x['bt_end_time']).subtract(x['otp_end_time']))) \
                                                                    .sort_values(['otp_user_trip_id', 'otp_itinerary_id', 'otp_leg_id', 'sched_obs_start_timediff', 'sched_obs_end_timediff'])

            scheduled_itin_observed_od['bt_duration_mins'] = (pd.to_datetime(
                scheduled_itin_observed_od['bt_end_time']) - pd.to_datetime(scheduled_itin_observed_od['bt_start_time']))/pd.Timedelta(minutes=1)
            ...
            output_bulma_otp=all_complete_vehicle_legs_options
            ...
            #     ['otp_leg_id', 'otp_mode', 'otp_route', 'bt_bus_code', 'bt_trip_num', 'otp_from_stop_id', 'otp_start_time', 'bt_start_time', 'sched_obs_start_timediff', 'otp_to_stop_id', 'otp_end_time', 'bt_end_time', 'sched_obs_end_timediff']).groupby(itineraries_legs_duplicates_dropped['otp_itinerary_id']).transform('any')]
